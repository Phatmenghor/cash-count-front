interface LoadingFullPage {
  loading?: boolean;
  className?: string;
  text?: string;
}

const LoadingFullPage: React.FC<LoadingFullPage> = ({
  loading = false,
  className = "",
  text = "Loading, please wait...",
}) => {
  if (!loading) return null; // If loading is false, hide the component

  return (
    <div
      className={`fixed inset-0 flex items-center justify-center ${className} bg-black bg-opacity-50 z-50`}
    >
      <div className="flex flex-col items-center bg-white px-6 py-8 rounded-xl">
        <svg
